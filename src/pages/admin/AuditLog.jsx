...
const ACTION_LABELS = {
  role_changed: { ar: "تغيير صلاحية", en: "Role Changed" },
  customer_blocked: { ar: "حظر عميل", en: "Customer Blocked" },
  customer_unblocked: { ar: "رفع حظر", en: "Customer Unblocked" },
  customer_updated: { ar: "تعديل عميل", en: "Customer Updated" },
  customer_created: { ar: "إضافة عميل", en: "Customer Created" },
};
  ...
  return (
    <div dir={dir} style={{ fontFamily: "'Cairo', sans-serif" }}>
      <div className="mb-6 flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-black">{t("Audit Log", "سجل التدقيق")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("Log of sensitive actions — owner only", "سجل الإجراءات الحساسة — للمالك فقط")}</p>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground font-bold">{t("No actions logged yet", "لا توجد إجراءات مسجّلة بعد")}</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Badge className="text-xs bg-amber-100 text-amber-700">
                        {ACTION_LABELS[log.action] ? t(ACTION_LABELS[log.action].en, ACTION_LABELS[log.action].ar) : log.action}
                      </Badge>
