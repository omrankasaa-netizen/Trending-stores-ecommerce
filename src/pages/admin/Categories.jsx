...
export default function AdminCategories() {
  ...
  const save = async () => {
    if (!form.name_ar) { toast({ title: "يرجى إدخال الاسم بالعربية", variant: "destructive" }); return; }
    setSaving(true);
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Button onClick={openNew} className="mt-4 rounded-xl">إضافة أول فئة</Button>
            </div>
          ) : (
              ...
              {categories.map(cat => (
                <div key={cat.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm">{cat.name_ar}</div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
          <DialogHeader>
            <DialogTitle className="font-black">{editing ? "تعديل الفئة" : "إضافة فئة جديدة"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              <Label className="font-bold">الاسم بالعربية *</Label>
              <Input className="mt-1 text-right" value={form.name_ar} onChange={e => f("name_ar", e.target.value)} style={{ direction: "rtl" }} placeholder="مثال: إلكترونيات" />
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">الرقم الأصغر يظهر أولاً</p>
            </div>
            <div>
              <Label className="font-bold">صورة الفئة</Label>
              {form.image_url ? (
                <div className="mt-1 flex items-center gap-3">
                  <img src={form.image_url} className="w-16 h-16 rounded-xl object-cover" />
                  <Button variant="outline" size="sm" onClick={() => f("image_url", "")}>إزالة</Button>
                </div>
              ) : (
                <label className="cursor-pointer block mt-1">
                  <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-primary hover:bg-primary/5 transition-colors">
                    ...
                    <p className="text-sm">{uploading ? "جاري الرفع..." : "رفع صورة"}</p>
                  </div>
            ...
            <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3">
              ...
              <span className="font-bold text-sm">ظاهرة في المتجر</span>
            </div>
          </div>
